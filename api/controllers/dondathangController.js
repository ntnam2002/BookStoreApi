...
exports.Editdondathang = function (req, res) {
    ...
    db.query(checkCustomerSQL, [req.body.makh], (err, customerInfo) => {
        ...
        if (!customerInfo || customerInfo.length === 0) {
            return res
                .status(404)
                .json({ message: "Không tìm thấy khách hàng có mã này" });
        }
        ...
        db.query(checkProductSQL, [req.body.masp], (err, productInfo) => {
            ...
            if (!productInfo || productInfo.length === 0) {
                return res
                    .status(404)
                    .json({ message: "Không tìm thấy sản phẩm có mã này" });
            }

            // Lấy số lượng hiện tại từ bảng dondathang
            let getCurrentQuantitySQL =
                "SELECT soluong FROM dondathang WHERE madh = ?";
            db.query(
                getCurrentQuantitySQL,
                [req.params.madh],
                (err, currentQuantityInfo) => {
                    if (err) {
                        console.error(err);
                        return res
                            .status(500)
                    ...
                    if (
                        !currentQuantityInfo ||
                        currentQuantityInfo.length === 0
                    ) {
                        return res.status(404).json({
                            message: "Không tìm thấy đơn đặt hàng có mã này",
                        });
                    }

                    // Tính chênh lệch giữa số lượng mới và số lượng hiện tại
                    const quantityDifferenceBeforeUpdate =
                        req.body.soluong - currentQuantityInfo[0].soluong;

                    // Tính tổng tiền dựa trên giá mới và số lượng mới của sách
                    const newTotalAmount =
                        req.body.soluong * productInfo[0].gia;

                    // Kiểm tra nếu sau khi update, số lượng sách trở nên âm, thì trả về lỗi
                    const updatedQuantity =
                        productInfo[0].soluong - quantityDifferenceBeforeUpdate;
                    if (updatedQuantity < 0) {
                        return res
                            .status(400)
                            .json({ message: "Không đủ sách để cập nhật" });
                    }

                    // Cập nhật số lượng trong bảng sách
                    let updateQuantitySQL =
                        "UPDATE sach SET soluong = ? WHERE masp = ?";
                    db.query(
                        updateQuantitySQL,
                        [updatedQuantity, req.body.masp],
                        (err, updateResponse) => {
                            if (err) {
                                console.error(err);
                                return res
                            ...
                            let updateOrderSQL =
                                "UPDATE dondathang SET makh=?, masp=?, soluong=?, tongtien=?, trangthai=?, pttt=? WHERE madh=?";
                            db.query(
                                updateOrderSQL,
                                [
                                    req.body.makh,
                                ...
                                (err, response) => {
                                    ...
                                    let getUpdatedQuantitySQL =
                                        "SELECT soluong FROM dondathang WHERE madh = ?";
                                    db.query(
                                        getUpdatedQuantitySQL,
                                        [req.params.madh],
                                        (err, updatedQuantityInfo) => {
                                            if (err) {
                                                console.error(err);
                                                return res.status(500).json({
                                                    message:
                                            ...
                                            if (
                                                !updatedQuantityInfo ||
                                                updatedQuantityInfo.length === 0
                                            ) {
                                                return res.status(404).json({
                                                    message:
                                                        "Không tìm thấy đơn đặt hàng có mã này",
                                                });
                                            }

                                            // Tính chênh lệch giữa số lượng mới và số lượng hiện tại sau khi update
                                            const quantityDifferenceAfterUpdate =
                                                updatedQuantityInfo[0].soluong -
                                                currentQuantityInfo[0].soluong;

                                            console.log(
                                                "Chênh lệch số lượng trước khi sửa đổi:",
                                                quantityDifferenceBeforeUpdate,
                                            );
                                            console.log(
                                                "Chênh lệch số lượng sau khi sửa đổi:",
                                                quantityDifferenceAfterUpdate,
                                            );
    ...
    try {
        ...
        db.query(checkCustomerSQL, [req.body.makh], (err, customerInfo) => {
            ...
            db.query(checkProductSQL, [req.body.masp], (err, productInfo) => {
                ...
                let getProductInfoSQL =
                    "SELECT gia, soluong AS currentQuantity FROM sach WHERE masp = ?";
                db.query(
                    getProductInfoSQL,
                    [req.body.masp],
                    (err, productInfo) => {
                        ...
                        if (requestedQuantity > currentQuantity) {
                            return res.status(400).json({
                                message:
                                    "Số lượng đặt hàng vượt quá số lượng hiện có",
                            });
                        }

                        // Tính tổng tiền dựa trên giá và số lượng của sách
                        const totalAmount =
                            requestedQuantity * productInfo[0].gia;
                        ...
                        db.query(
                            ...
                            (err, response) => {
                                ...
                                // Cập nhật số lượng trong bảng sách
                                let updateQuantitySQL =
                                    "UPDATE sach SET soluong = soluong - ? WHERE masp = ?";
                                db.query(
                                    updateQuantitySQL,
                                    [requestedQuantity, req.body.masp],
                                    (err, updateResponse) => {
                                        if (err) {
                                            console.error(err);
                                            return res.status(500).json({
...
exports.Deletedondathang = function (req, res) {
    ...
    let getDondathangInfoSQL =
        "SELECT masp, soluong FROM dondathang WHERE madh = ?";
    db.query(getDondathangInfoSQL, [req.params.madh], (err, dondathangInfo) => {
        ...
        if (!dondathangInfo || dondathangInfo.length === 0) {
            return res
                .status(404)
                .json({ message: "Đơn đặt hàng không tồn tại" });
        }
        ...
        db.query(deleteDondathangSQL, [req.params.madh], (err, response) => {
            ...
            db.query(
                updateSachSQL,
                [dondathangInfo[0].soluong, dondathangInfo[0].masp],
                (err, updateResponse) => {
                    if (err) {
                        console.error(err);
                        return res
                    ...
                    res.json({
                        message:
                            "Đã xóa đơn đặt hàng và cập nhật số lượng sách",
                    });
                },
            );
        });
...
exports.GetAlldondathangWithCustomerInfo = function (req, res) {
    let sql =
        "SELECT dondathang.*, khachhang.makh, khachhang.hoten, khachhang.sdt, khachhang.email, khachhang.diachi " +
        "FROM dondathang " +
